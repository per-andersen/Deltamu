...
######## FlatJBP_CDM
def fjbpcdm_inv_efunc(double z, double Om0, double Ode0,
    double Ogamma0, double w0,
    double wa):

  cdef double opz = 1.0 + z
  cdef Odescl = opz**(3. * (1 + w0)) * exp(1.5 * wa * z * z / (opz*opz) )
  return pow(Om0 * opz**3 + Ode0 * Odescl, -0.5)

######## Flatn3_CDM
def fn3cdm_inv_efunc(double z, double Om0, double Ode0,
    double Ogamma0, double w0,
    double wa):

  cdef double opz = 1.0 + z
  cdef double aa = pow(opz, -1)
  cdef Odescl = opz**(3. * (1 + w0 + wa)) * exp(-0.5*wa * (1.-aa)*(2.*aa*aa-7.*aa+11.))
  return pow(Om0 * opz**3 + Ode0 * Odescl, -0.5)

######## Flatn7_CDM
def fn7cdm_inv_efunc(double z, double Om0, double Ode0,
    double Ogamma0, double w0,
    double wa):

  cdef double opz = 1.0 + z
  cdef double aa = pow(opz, -1)
  cdef Odescl = opz**(3. * (1 + w0 + wa)) * exp(-wa * (1.-aa)*(60*pow(aa,6)-430*pow(aa,5)+1334*pow(aa,4)-2341*pow(aa,3)+2559*aa*aa -1851*aa+1089)/140.)
  return pow(Om0 * opz**3 + Ode0 * Odescl, -0.5)
